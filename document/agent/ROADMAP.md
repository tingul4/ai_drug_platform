...
- [x] **`load_candidates()` 為 peptide-aware**（2026-04-25）：依輸入序列每個 K 位置產生候選，有真實 ala scan 資料時用真實 ΔΔG，缺少時 fallback 到 Stefansson anchor。詳見 `document/agent/PHASE1_2_BoltzMMGBSA_alignment.md`
- [x] **`run_alanine_scan()` 接上真實批次**（2026-04-25）：`mmgbsa` 走 in-place K→A 路徑（單一 WT pose、stripping 側鏈），`iptm` 走 refold 路徑；三條 demo peptide 都已預跑入 cache
- [x] **ρ 改為真實 + signed**（2026-04-25）：原本是讀 hardcoded `_STEFANSSON_DDG` 表必然 +1，且 `abs(rho)` 把 PE1020 結構訊號吃掉；現在從 ala scan EEL ddG 計算（避開 GB charge cancellation）並保留正負號
- [x] **Tool B 加 iptm 選項**（2026-04-25）：對齊 PDF 6 方法 × 3 peptide benchmark
...
- [ ] **Boltz seed 平均**：目前 single-seed pose 對 PE1016/PE1020 的 ranking 還沒對齊 PDF；多 seed 取共識能修正單 pose 不穩
...
- 2026-04-25 (晚)：對齊 `Aim3_OnePager_BoltzMMGBSA.pdf`。Tool B 加 iptm；ala scan 改 in-place K→A（Phase 2，PDF 方法學）；ρ 改用 EEL ddG 計算且保留正負號；WT cache 路徑 bug 修正。實作細節寫在 `PHASE1_2_BoltzMMGBSA_alignment.md`。
